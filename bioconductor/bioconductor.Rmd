...
> Answer:
> browseVignettes("DESeq2"), but this doesn't work on the ccb cluster, instead you can do through the help menu.
...
```{r}
kevin@age # don't use @ unless you have to, it is a short cut to access 'slots', as there should be a way of getting that information through another function. This is because the developers can change the way they store the information, e.g. instead of age they may change it to date of birth. The function below 'get_age' wouldn't change and would produce the same age, but it would @dateofbirth, rather than @age. Therefore your personal function and code would break if you used @age.
```
...
```{r, eval=FALSE}
help(DNA_ALPHABET) #this is the help page for this object
```
...
```{r}
#look at the help pdf
seqnames(BSgenome.Hsapiens.UCSC.hg38.masked)
chrY <- BSgenome.Hsapiens.UCSC.hg38.masked$chrY
...
```{r}
gtf_data <- rtracklayer::import("/t1-data/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/Homo_sapiens.GRCh38.104.gtf.gz")
gtf_data
```
...
unique_ACTB <- unique(gtf_ACTB$transcript_id)

unique_ACTB

#then you need to remove the NA (nb you can't use tidyverse functions on this)

unique_ACTB <- na.omit(unique_ACTB)
...
lengths(gtf_ACTB_transcripts) #lengths() returns the number of inputs of each item(list) of your list
...
```{r, include=TRUE}
columns(org.Hs.eg.db) #in this database everything is a gene and these are all the pieces of data that we can extract.
```
...
```{r}
human_symbols <- head(keys(org.Hs.eg.db, keytype = "SYMBOL")) #head() selects top 6 results
...
# gene symbol is the unique annotation for each gene.
# some packages only take 1 or other of the symbol, entrezid, or ensembl ID - they are redunant of each other. When writing up your papers you should refer to the gene symbol.
```
...
```{r}
mapIds(org.Hs.eg.db, keys=human_symbols, column='ENSEMBL', keytype='SYMBOL') #used the example in the help page to figure this out.
```
