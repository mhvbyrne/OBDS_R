...
```{r}
library(DropletUtils)
sce <- DropletUtils::read10xCounts(samples = c("pbmc5k"="/project/obds/shared/resources/4_r_single_cell/singlecell_droplets/raw_feature_bc_matrix"), col.names=TRUE) #col.names are the cell barcodes, rownames are the genes
...
head(sce) #here we can see there is an assay called counts
assay(sce, "counts") #then we can visualise the matrix

sce <- sce[, colSums(assay(sce, "counts"))>0] #then we need to remove all the columns that have a sum >0, this is because they contain no counts for any genes. It is colsum rather than rowsums (which is what we used in a previous example because we are now using bioconductor which is transposed)
...
out <- DropletUtils::emptyDrops(sce,
                                lower=100,
                                niters=10000,
                                assay.type="counts") #importantly you don't assign this back to the object.
out #FDR is the false discovery rate, it is a form of multiple testing correcting
```
...
```{r}
out$FDR<0.01 # this gives us true and false values if FDR <0.01
positions <- which(out$FDR<0.01) #we can then use which() to give us the cell values for the TRUE values.
sce_filtered_emptydrops <- sce[,positions] #this also filters out NA as R takes NAs as FALSE.
...
```{r}
str(sce)
plot_data <- tibble(librarysize = colSums(assay(sce, "counts")), emptydroplets = out$FDR<0.01)
  
plot_data
ggplot(plot_data, aes(x=emptydroplets, y=(librarysize))) + geom_violin() + scale_y_log10() #it is important to log transform this as there is a large difference between 1 UMI and 10,000 UMI and the graph is unreadable otherwise.
...
table(sce_scdblfinder$scDblFinder.class) #you can see the column names using colData
str(sce_scdblfinder)
```
...
plotDimReduceCluster(sce_decontX, dim1 = umap[, 1], dim2 = umap[, 2])
```
