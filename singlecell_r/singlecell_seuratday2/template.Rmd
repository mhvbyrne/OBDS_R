...
```{r}
seurat_after_qc <- readRDS("/project/obds/shared/resources/4_r_single_cell/singlecell_seuratday2/seurat_after_qc.rds") #this data is the same as the data we have just finished working on from day1. it's just standardised across everyone.
```
...
seurat_after_qc[[]] #this is a quick way to access the meta data
```
...
VlnPlot(seurat_after_qc,
        ...
        same.y.lims = TRUE #this makes the y axis the same
)
...
#These are clustered based on our previous clusters
#You can also remove the clustering using 
VlnPlot(seurat_after_qc,
        features = c("nCount_RNA", "nCount_SCT"),
        same.y.lims = TRUE, #this makes the y axis the same,
        group.by = "orig.ident" #this is the same as the first one but without clustering, and it is done across all samples
)
...
```{r}
#there are some differences between the log method we used before and the SCT normalisation - one is not definitely better - but you can try another method if the clustering hasn't worked well using a different method. 
```
...
```{r}
DefaultAssay(seurat_after_qc) #this is the default assay
DefaultAssay(seurat_after_qc) <- "SCT" #this is how you assign what the default assay is.
seurat_after_qc <- RunPCA(seurat_after_qc, reduction.name = "sct.pca") #you need to change the reducation name so that you can keep the rna assay pca that we ran before
```
...
p1 + p2 #the clustering for these is both from the RNA assay
```
...
seurat_after_qc <-  FindNeighbors(seurat_after_qc,
                                  ...
                                  ) #this may overwrite the previous graph you have
```
...
```{r}
Graphs(seurat_after_qc) #this is how you figure out what the names of the graphs you have made are.
...
```{r}
table(seurat_after_qc$RNA_snn_res.0.5, seurat_after_qc$SCT_snn_res.0.5) #this gives the number of cells that are in each cluster for your two
...
```{r}
p3 <- DimPlot(seurat_after_qc, reduction = "sct.umap", group.by="SCT_snn_res.0.5") + ggtitle("SCT UMAP") #this is the clusters
p1 + p3 #p1 is already plotted
...
feat1 <- FeaturePlot(seurat_after_qc, features = c("LYZ","FCGR3A","CD4","CD8A","GNLY","MS4A1","CST3","PPBP"), reduction = "umap")
feat2 <- FeaturePlot(seurat_after_qc, features = c("LYZ","FCGR3A","CD4","CD8A","GNLY","MS4A1","CST3","PPBP"), reduction = "sct.umap")
...
```{r}
#before running this you need to run the command to make sure you are using the correct idents following:
#Idents(seurat_markers_all) <- "RNA"
...
head(hto12.htos) #each row is a barcode(cell)
dim(hto12.htos)

#this is the matrix that has the different antibody tag that were given to the different samples (e.g. HEK_A). There is a lot of noise, which is why in the same cell there are multiple antibody reads - even though there should only be a single one that is positive.
```
...
```{r}
joint.bcs <- intersect(rownames(hto12.htos),colnames(hto12.umis)) #this gives us a vector of all the cells that are in both,
str(joint.bcs)
```
...
dim(hto12.umis.common) #check it matches the subset you made previously
...
```{r}
hto12.htos.common <- t(hto12.htos[colnames(hto12_object), #these are the cell barcodes, need to select the colnames
                                1:12 #these are the HTO Tags the other columns are metadata
                                ]) #t is transpose
dim(hto12.htos.common) #head doesn't work well as there are 8533 columns and it tries to show you all the columns
...
hto12_object <- NormalizeData(hto12_object, assay = "HTO", normalization.method = "CLR" , margin=1) #the data is bimodal that's why we use CLR
```
...
hto12_object <- HTODemux(hto12_object,
                         assay = "HTO") #positive quantile it the p value cut off, default is 0.99
```
...
```{r}
hto12_object[[]] #HTO_maxID is the most probable label that has been assigned, HTO margin is a score - it is essentially the difference between these two. The margin is then used to assign whether it is a doublet or a singlet. If there's not much of a difference it's probably a doublet. The hash.ID is the final assignment or whether it was a double.
```
...
RidgePlot(hto12_object, assay = "HTO", features = c("HEK-A","KG1-A","THP1-A","K562-A")) #this allows you to visualise the signal from the background noise. This is on a log scale.
```
...
FeatureScatter(hto12_object, feature1 = "HEK-A", feature2 = "HEK-B", group.by = "HTO_classification.global") #the ones in the top right are the doublets with both HEKA and HEKB
FeatureScatter(hto12_object, feature1 = "HEK-A", feature2 = "HEK-B", group.by = "HTO_maxID")
FeatureScatter(hto12_object, feature1 = "HEK-A", feature2 = "HEK-B", group.by = "HTO_secondID")
```
what do you notice ?
